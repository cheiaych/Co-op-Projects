...
public class ConfigTester {
	...
	public static File untrusted = fileChooser("Choose untrusted configuration file directory...");
	...
	public static File output = new File (trusted.getParentFile().getAbsolutePath() + "\\ConfigTestOutput.csv");
...
//////////////////////////////////////////////////////////////////
	...
	public static boolean ruleChecker(Packet P, int index, PrintWriter pw) throws Exception
	{
		...
		if (XMLBroadIPRange.equals(TrustInternalIP))
		{
			...
			{	
				...
				if (subnetRange(P, leftSubnet, TrustExternalIP, leftSubnetRange))
				{
					...
					if ((rule = findNewDest(P) == true))
					{
						...
						if (findConnection(P) == true)
						{
							...
							else
							{
								...
								out = ("ERROR: 'right-ip and TCV_PARTNER_LINK_ADDR are not in same subnet'");
								rule = false;
						...
						{
							System.out.print("ERROR: 'Destination IP is not in Right Subnet Range'");
							out = ("ERROR: 'Destination IP is not in Right Subnet Range'");
							rule = false;
					...
					{
						System.out.print("ERROR: 'Destination port and ID not found'");
				...
				{
					System.out.print("ERROR: 'ExternalIP is in not Left Subnet Range'");
					out = ("ERROR: 'ExternalIP is in not Left Subnet Range'");
					rule = false;
			...
			else
			{
				...
				out = ("LeftIP, LeftIDIP, and TCV Partner Link Address do not match");
				rule = false;
		...
		else
		{
			...
			out = ("BroadcastIP matches Internal IP");
			rule = false;
