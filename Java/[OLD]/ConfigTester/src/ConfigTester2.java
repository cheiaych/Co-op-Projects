...
public class ConfigTester2 {
	...
	public static class Packet
	{
		...
		Packet (String nID, String nIP, String nPort)
		{
			DestIP = nIP;
			DestPort = nPort;
			DestID = nID;
		...
		if (UntrustTcvBridgeAddr.equals(tcvPartAddr))
		{
			...
			if (subnetRange(P, tcvPartLinkAddr, Connections.get(connectionNo).rightip, subnetMaskToRange(tcvPartNetMask.toCharArray())))
			{
				...
				if (findConnection(P) == true)
				{
					...
					if ((rule = findBroadDestination(P) == true))
					{
						...
						if (subnetRange(P, leftSubnet, TrustExternalIP, leftSubnetRange))
						{
							...
							{
								System.out.println("New Packet Info: Internal Port: " + P.IntPort + " Destination IP: " + P.DestIP + " Destination Port: " + P.DestPort);
							}
							else
							{
	...
	public static void main(String[] args) throws Exception {
		...
		String IP, Port, ID, trustedFilePath, untrustedFilePath;
		int i;
		...
		//while (i <= 2)
		{
			...
			Packet P = new Packet (ID, IP, Port);
